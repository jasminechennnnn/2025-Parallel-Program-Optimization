...
void print_alignment(const AlignmentResult& result, 
    ...
    cout << "SIMD Speedup: " << fixed << setprecision(1) << (baseline_time / simd_time) << "X" << endl;
    cout << "-------------------------------" << endl;
    cout << "CUDA time: " << fixed << setprecision(1) << cuda_time << " ms" << endl;
    cout << "CUDA Speedup: " << fixed << setprecision(1) << (baseline_time / cuda_time) << "X" << endl;
    cout << "-------------------------------" << endl;
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < query_sequences.size(); ++i) {
        ...
        // Run CUDA-accelerated alignment
        auto start_cuda = chrono::high_resolution_clock::now();
        AlignmentResult cuda_result = smith_waterman_striped_cuda(
            target_seq.sequence, query_seq.sequence, scoring_matrix);
        auto end_cuda = chrono::high_resolution_clock::now();
        double cuda_time = chrono::duration<double, milli>(
            end_cuda - start_cuda).count();
        
        // Print the CUDA result
        cout << "\n--------CUDA result for target: " << target_seq.name << "--------" << endl;
        print_alignment(cuda_result, target_seq.name, query_seq.name,
                    target_seq.sequence, query_seq.sequence, baseline_time, simd_time, cuda_time);

        // Print the XSIMD result
